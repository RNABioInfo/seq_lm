...
def _write_count(path: Path, reads: int) -> None:
    path.write_text(f"tname\tlen\tnum_reads\ntx1\t100\t{reads}\n")


def _run_check(tmp_path: Path, reads: list[int]) -> str:
    counts_dir = tmp_path / "counts"
    counts_dir.mkdir()
    rows = []
    for index, read_count in enumerate(reads):
        count_file = f"sample_{index}.quant"
        _write_count(counts_dir / count_file, read_count)
        rows.append(
            {
...
def test_stability_read_count_requires_enough_deep_samples_per_group(tmp_path):
    assert _run_check(tmp_path, [10000, 12000, 11000, 9000]) == "false"


def test_stability_read_count_accepts_all_groups_at_threshold(tmp_path):
    assert _run_check(tmp_path, [10000, 12000, 11000, 13000]) == "true"
